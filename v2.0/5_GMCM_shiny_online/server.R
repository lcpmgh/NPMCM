...
server <- function(input, output){
  ...
  showtext_auto()
  font_add("myFont", './front/msyh.ttc')    #导入中文字体
  
  ########## 数据区 ##########
  read_data_team <- reactive({
    # 读取获奖队伍数据
    # data_team <- fread(file_dir('award_team.csv')) %>% 
    data_team <- fread('https://raw.githubusercontent.com/lcpmgh/GMCM/master/v2.0/data-GB2312/award_team.csv') %>% 
      .[, award_type:=factor(award_type, levels = c('一等奖','二等奖','三等奖'), order = T)]
    return(data_team)
  })
  
  read_data_member <- reactive({
    # 读取获奖队伍数据
    # data_member <- fread(file_dir('award_member.csv')) %>% 
    data_member <- fread('https://raw.githubusercontent.com/lcpmgh/GMCM/master/v2.0/data-GB2312/award_member.csv') %>% 
      .[, award_type:=factor(award_type, levels = c('一等奖','二等奖','三等奖'), order = T)]
    return(data_member)
  })
  ...
  read_map_data <- reactive({
    ...
    province_city <- fread('https://raw.githubusercontent.com/lcpmgh/GMCM/master/v2.0/data-GB2312/province_capital.csv')
