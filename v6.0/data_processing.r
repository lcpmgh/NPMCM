...
ver <- "6.0"  #定义正在修改数据的版本，用于存储、读取数据时，定位文件夹                                                                 
# setwd(choose.dir(default = 'D:/#R/@github/NPMCM'))                                          #将临时路径指定到NPMCM文件夹
...
data_collection <- function(){
  ...
  for(file_name in files_name){
    t <- my_read_csv(file_name, raw_data = T, integer64 = 'character', select = 1:9)          #转换队号列数据类型，并排除2020年之后的备注列
  ...
  total_data <- dplyr::rename(total_data, 
    ...
    .[!(A_type %in% c('一等奖', '二等奖', '三等奖')), A_type:='成功参与奖']                   #变量重命名、生成唯一的队伍id、归一未获的奖项名为“成功参与奖”
  message(sprintf('已整合以上【%d份】获奖名单文件。', length(files_name)))
  return(total_data)
}
...
unit_standardization <- function(total_data){
  ...
  for(i in 1:nrow(unit_name_convert)){
    ...
    if(nrow(check)==0){
      # message(sprintf('-----未找到【%s】', o_name)) #不显示无效项
      next()
    }
...
unit_info_genrating <- function(crawler = F){
  ...
  unit_info_dataset <- dplyr::full_join(unit_info_dataset, data_unit, by='Unit')                         #与获奖名单中单位的合并
  unit_info_dataset_na <- unit_info_dataset[is.na(Country), ]    
  unit_info_dataset_re <- unit_info_dataset[!is.na(Country), ] %>% setorder(Unit)                        #无缺失值部分按名称排序
  unit_info_dataset <- rbind(unit_info_dataset_na, unit_info_dataset_re)
...
member_data_generating <- function(total_data){
  ...
  member_data <- total_data %>% 
    .[A_type %in% c('一等奖','二等奖','三等奖'),] %>%    #若不排除"成功参与奖"数据量大且耗时
...
# 4.2 培养单位名称归一化（重复运行直至无跳出替换信息截止，因为有时一次替换不完）
total_data <- unit_standardization(total_data)                       #单位名称标准化
# 4.3 生成培养单位信息（初步，后续需人工查验补齐）
# 网络爬虫生成的本地数据库，已经覆盖大部分培养单位信息，且网络上的信息更新周期很长，因此后续使用一般不用再爬虫
# 手动查漏补缺步骤：
# 此函数，若输出结果中，存在NA值，则有两种情况：1有新的培养单位参加，2旧的培养单位出现新的不规范名字
# 对于1需要修改3-unit_info_dataset.csv文件，添加新培养单位的信息，或者，添加新标准名称并添加新标准化规则
# 对于2需要修改0-unit_name_convert.csv文件，添加新的培养单位标准化规则
# 至于是1还是2，将unit_info_dataset保存为999-temp-unit_info_dataset.csv挨个查看
# 如果某个country和province为NA的unit，向下找，找不到同单位的，说明是1，找到说明是2
# 由于早期编辑不严谨，在3-unit_info_dataset.csv中，也有可能有重名的，此时需要删掉一个不规范的行
# 结束这两步后，重新运行4.1和4.2，直至此函数运行结果中无NA值为止
unit_info_dataset <- unit_info_genrating(crawler = F);unit_info_dataset
my_write_csv(unit_info_dataset, "999-temp-unit_info_dataset.csv")   #临时储存以便查看结果
# 4.4 生成获奖名单（处理后的）中的单位名称集（唯一化）（用于培养单位名称标准化手动的查漏补缺）
