server <- function(input, output){
  ...
  p2_data0 <- data_team %>%
    ...
    .[, c(1,5,6)] %>% 
    set_colnames(c("Year", "c", "s")) %>% 
    .[c==0, s:=NA_integer_] %>% 
    as.data.frame() %>%
    set_rownames(., .[,1]) %>%
    .[3] %>% 
    set_colnames(c("参赛队伍量"))
  renderLineChart(div_id = "p2_plot0",  data = p2_data0, axis.x.name = '年份', axis.y.name = '数量', theme = 'shine')
  p2_data1 <- data_team %>%
    dcast(Year~A_type, value.var = 'Year', fun = length) %>%
    .[, s:=apply(.SD, 1, sum), .SDcols=2:5] %>%
